...
#define TIM2CLK 8000000 // 8 MHz
#define F_SIGNAL 1000   // 1 kHz
/* USER CODE END PD */
...
volatile Waveform current_waveform = SINE_WAVE;
/* USER CODE END PV */
...
void generate_LUTs(void) {
    for (int i = 0; i < NS; i++) {
        // Sinusoidal LUT
        Sin_LUT[i] = (uint32_t)((sin((2 * M_PI * i) / NS) + 1) * 511.5); // Scaled to 0-1023
...
int main(void)
{
  ...
  // Generate LUTs and calculate TIM2_Ticks
  generate_LUTs();
  ...
  // Start DMA in Interrupt (IT) mode with Sine LUT as source and TIM3->CCR3 as destination
